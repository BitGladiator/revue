...
const runAgent = async (agentName, systemPrompt, userContent, model = FAST_MODEL) => {
  ...
  try {
    ...
    console.log(`[${agentName}] done in ${Date.now() - start}ms, tokens: ${tokens}`);
...
`.trim();
  ...
  const totalTokens =
    agentResults.reduce((s, r) => s + r.tokens, 0) + (aggregatorResult.tokens || 0);
